  ...
  return (
    <div className="w-full relative flex flex-col items-center">
      ...
      <div className="relative group w-full max-w-xl flex items-center transition-all duration-500 ease-out">
        ...
        <div className="flex-1 flex items-center bg-black/40 backdrop-blur-3xl border border-white/10 rounded-full shadow-2xl focus-within:bg-black/60 focus-within:border-white/20 focus-within:-translate-y-[2px] transition-all duration-300 overflow-hidden pl-2 pr-2 py-1">
          
          {setMode && (
            <select
              value={mode}
              onChange={(e) => setMode(e.target.value as 'AUTO' | 'FAST' | 'DEEP')}
              className="bg-transparent text-white/40 hover:text-white border-0 text-[10px] font-mono tracking-widest cursor-pointer px-2 outline-none appearance-none transition-all ml-2"
          ...
          <button 
            ...
            className="p-3 rounded-full text-white/40 hover:text-white hover:bg-white/10 transition-all flex-shrink-0"
          >
          ...
          <textarea 
            ...
            className="flex-1 max-h-32 bg-transparent text-white font-light tracking-wide px-3 py-2.5 resize-none focus:outline-none placeholder-white/30 text-[15px] leading-relaxed self-center flex items-center custom-scrollbar"
            rows={1}
          ...
          <div className="p-1 flex-shrink-0">
            {isProcessing ? (
              <button 
                ...
                className={`p-3 rounded-full transition-all border ${isCancelling ? 'bg-white/5 text-white/30 border-white/5 cursor-not-allowed' : 'bg-white/10 text-white hover:bg-white/20 border-white/10'}`}
              >
              ...
              <button 
                type="button"
                onClick={onSubmit}
                className="p-3 bg-white text-black rounded-full hover:bg-gray-200 transition-all border border-transparent shadow-lg"
              >
