...
export class OpenAICompatibleProvider implements ModelProvider {
  ...
  async generateEmbeddings(texts: string[], options?: { modelOverride?: string }): Promise<number[][]> {
    const url = `${this.config.baseUrl}/embeddings`;
    const payload = {
      model: options?.modelOverride || 'nomic-embed-text',
      input: texts,
    };
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`OpenAI Provider Embedding Error: ${response.status} ${response.statusText} - ${errorText}`);
    }

    const data = await response.json();
    // Assuming data.data is an array of objects with an 'embedding' property
