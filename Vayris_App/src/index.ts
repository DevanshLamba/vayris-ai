...
// Load environment variables handled in startServer
// dotenv.config();

export async function startServer(userDataPath?: string, appPath?: string) {
  console.log('Initializing VAYRIS...');

  const basePath = userDataPath || process.cwd();
  const applicationPath = appPath || process.cwd();
  ...
  if (process.env.VAYRIS_TERMS_ACCEPTED !== 'true') {
    throw new Error('NEEDS_LEGAL');
  }
  ...
  const memoryDbPath = path.resolve(basePath, 'vayris_memory.db');
  const memoryStore = new SQLiteMemoryStore(memoryDbPath);

  const ragDbPath = path.resolve(basePath, 'vayris_rag.db');
  ...
  await ProductivityStorage.getInstance().initialize(path.resolve(basePath, 'vayris_productivity.db'));
  ...
  const mcpConfigPath = path.resolve(basePath, 'mcp.config.json');
  ...
  try {
    ...
    if (e.message.startsWith('MISSING_MODELS') || e.message === 'NEEDS_SETUP') throw e;
  ...
  const server = new VayrisServer(orchestrator, toolContext, basePermissionManager, memoryStore, applicationPath, 3000);
  ...
  return { server, orchestrator, memoryStore, mcpManager, browserManager, vectorStore };
}
