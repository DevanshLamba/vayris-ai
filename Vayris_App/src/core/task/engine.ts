...
2. You cannot fabricate or self-attest evidence. Evidence provided to complete_task MUST be based on the actual tool responses you received.` 
    ...
    if (state.metadata.domain === 'productivity') {
      const allowed = ['tasks_list', 'tasks_create', 'tasks_complete', 'tasks_delete', 'tasks_count', 'reminders_list', 'reminders_create', 'reminders_cancel', 'alarms_list', 'alarms_create', 'alarms_cancel', 'email_draft', 'complete_task', 'fail_task'];
    ...
    while (iterations < this.maxIterations) {
      ...
      if (!response.tool_calls || response.tool_calls.length === 0) {
        if (typeof response.content === 'string' && (response.content.includes('"name"') || response.content.includes('"function"'))) {
          try {
            ...
            if (!hasValidJson && (potentialJsonStr.includes('"type":"function"') || potentialJsonStr.includes('"type": "function"'))) {
                // It is definitely a raw tool call that failed parsing due to LLM syntax error (e.g. invalid json).
                // We must hide this from the user. We will create a dummy tool call to trigger a re-prompt or error safely.
