...
exports.productivityTools = exports.EmailDraftTool = exports.RemindersUpdateTool = exports.RemindersCancelTool = exports.AlarmsCancelTool = exports.AlarmsCreateTool = exports.AlarmsListTool = exports.RemindersCreateTool = exports.RemindersListTool = exports.TasksCountTool = exports.TasksDeleteTool = exports.TasksUpdateTool = exports.TasksCompleteTool = exports.TasksCreateTool = exports.TasksListTool = void 0;
const types_1 = require("../../types");
const tasks_1 = require("./tasks");
const reminders_1 = require("./reminders");
const alarms_1 = require("./alarms");
class TasksListTool extends types_1.BaseTool {
    name = 'tasks_list';
    ...
    async execute(args) {
        ...
        const tasks = await tasks_1.TaskService.list({ status: requestedStatus });
        if (tasks.length === 0)
            return { success: true, data: tasks, uiContent: "You're all clear, Boss. No pending tasks.", spokenContent: "You're all clear, Boss. No pending tasks." };
...
exports.TasksListTool = TasksListTool;
class TasksCreateTool extends types_1.BaseTool {
    name = 'tasks_create';
    ...
    async execute(args) {
        try {
            ...
            const task = await tasks_1.TaskService.create({
                title: args.title,
                notes: args.notes,
...
exports.TasksCreateTool = TasksCreateTool;
class TasksCompleteTool extends types_1.BaseTool {
    name = 'tasks_complete';
    ...
    async execute(args) {
        ...
        if (/^\d+$/.test(targetId)) {
            ...
            const tasks = await tasks_1.TaskService.list({ status: 'pending' });
            if (tasks[index])
                targetId = tasks[index].id;
            else
                return { success: false, uiContent: `<div class="text-rose-400">Task #${targetId} not found in pending list.</div>`, spokenContent: `Task ${targetId} not found.` };
        }
        const task = await tasks_1.TaskService.complete(targetId);
        const tasksTool = new TasksListTool();
        const ui = await tasksTool.execute({ status: 'pending' });
...
exports.TasksCompleteTool = TasksCompleteTool;
class TasksUpdateTool extends types_1.BaseTool {
    name = 'tasks_update';
    ...
    async execute(args) {
        ...
        if (args.dueAt) {
            ...
            if (d)
                updateData.dueAt = d.toISOString();
        }
        const task = await tasks_1.TaskService.update(targetId, updateData);
        const tasksTool = new TasksListTool();
        const ui = await tasksTool.execute({ status: 'pending' });
...
exports.TasksUpdateTool = TasksUpdateTool;
class TasksDeleteTool extends types_1.BaseTool {
    name = 'tasks_delete';
    ...
    async execute(args) {
        ...
        await tasks_1.TaskService.delete(targetId);
        const tasksTool = new TasksListTool();
        const ui = await tasksTool.execute({ status: 'pending' });
...
exports.TasksDeleteTool = TasksDeleteTool;
class TasksCountTool extends types_1.BaseTool {
    name = 'tasks_count';
    description = 'Count tasks.';
    permissionLevel = types_1.PermissionLevel.SAFE;
    parameters = { type: 'object', properties: {} };
    async execute(args) {
        const tasks = await tasks_1.TaskService.list();
        const pending = tasks.filter(t => t.status === 'pending').length;
        const completed = tasks.filter(t => t.status === 'completed').length;
        return { success: true, data: { total: tasks.length, pending, completed } };
    }
}
exports.TasksCountTool = TasksCountTool;
class RemindersListTool extends types_1.BaseTool {
    name = 'reminders_list';
    description = 'List all reminders.';
    permissionLevel = types_1.PermissionLevel.SAFE;
    parameters = { type: 'object', properties: {} };
    async execute(args) {
        const items = await reminders_1.ReminderService.list();
        if (items.length === 0)
            return { success: true, data: items, uiContent: "You have no reminders." };
...
exports.RemindersListTool = RemindersListTool;
class RemindersCreateTool extends types_1.BaseTool {
    ...
    async execute(args) {
        try {
            ...
            const r = await reminders_1.ReminderService.create({
                title: args.title,
...
exports.RemindersCreateTool = RemindersCreateTool;
class AlarmsListTool extends types_1.BaseTool {
    name = 'alarms_list';
    description = 'List all alarms.';
    permissionLevel = types_1.PermissionLevel.SAFE;
    parameters = { type: 'object', properties: {} };
    async execute(args) {
        const items = await alarms_1.AlarmService.list();
        if (items.length === 0)
            return { success: true, data: items, uiContent: "You have no alarms." };
...
exports.AlarmsListTool = AlarmsListTool;
class AlarmsCreateTool extends types_1.BaseTool {
    name = 'alarms_create';
    ...
    async execute(args) {
        try {
            ...
            const a = await alarms_1.AlarmService.create({
...
exports.AlarmsCreateTool = AlarmsCreateTool;
class AlarmsCancelTool extends types_1.BaseTool {
    name = 'alarms_cancel';
    ...
    async execute(args) {
        ...
        if (targetId && /^\d+$/.test(targetId)) {
            const index = parseInt(targetId, 10) - 1;
            if (alarms[index])
                targetId = alarms[index].id;
            else
                return { success: false, uiContent: `<div class="text-rose-400">Alarm #${targetId} not found.</div>`, spokenContent: `Alarm ${targetId} not found.` };
        }
        if (!targetId) {
            if (alarms.length === 1) {
        ...
        await alarms_1.AlarmService.cancel(targetId);
        require('./scheduler').ProductivityScheduler.getInstance().scheduleNext();
...
exports.AlarmsCancelTool = AlarmsCancelTool;
class RemindersCancelTool extends types_1.BaseTool {
    ...
    async execute(args) {
        ...
        if (targetId && /^\d+$/.test(targetId)) {
            const index = parseInt(targetId, 10) - 1;
            if (reminders[index])
                targetId = reminders[index].id;
            else
                return { success: false, uiContent: `<div class="text-rose-400">Reminder #${targetId} not found in scheduled list.</div>`, spokenContent: `Reminder ${targetId} not found.` };
        }
            ...
            if (match)
                ...
                return { success: false, uiContent: `<div class="text-rose-400">Reminder matching "${args.title}" not found.</div>`, spokenContent: `I couldn't find a reminder matching ${args.title}.` };
        }
        if (!targetId)
            return { success: false, uiContent: `<div class="text-rose-400">Please provide a reminder ID or title to cancel.</div>`, spokenContent: "Please provide a reminder ID." };
        await reminders_1.ReminderService.cancel(targetId);
        require('./scheduler').ProductivityScheduler.getInstance().scheduleNext();
...
exports.RemindersCancelTool = RemindersCancelTool;
class RemindersUpdateTool extends types_1.BaseTool {
    ...
    async execute(args) {
        ...
        if (targetId && /^\d+$/.test(targetId)) {
            ...
            if (match)
                ...
                return { success: false, uiContent: `<div class="text-rose-400">Reminder matching "${args.searchTitle}" not found.</div>`, spokenContent: `I couldn't find a reminder matching ${args.searchTitle}.` };
        }
        if (!targetId)
            return { success: false, uiContent: `<div class="text-rose-400">Please provide a reminder ID or searchTitle to update.</div>`, spokenContent: "Please provide a reminder ID." };
        const updateData = {};
        if (args.title)
            updateData.title = args.title;
        if (args.remindAt) {
            ...
            const parsed = parseAdvancedTime(args.remindAt);
            if (!parsed)
                return { success: false, uiContent: `<div class="text-white/90 font-light">I couldn't understand the new time.</div>`, spokenContent: "I couldn't understand the time." };
            if (!parsed.hasTime)
                return { success: false, uiContent: `<div class="text-white/90 font-light">What time, Boss?</div>`, spokenContent: "What time, Boss?" };
            const d = parsed.date;
            if (d.getTime() < Date.now() - 60000)
                return { success: false, uiContent: `<div class="text-rose-400 font-light">That time is in the past!</div>`, spokenContent: "That time is in the past!" };
            updateData.remindAt = d.toISOString();
        }
        await reminders_1.ReminderService.update(targetId, updateData);
        require('./scheduler').ProductivityScheduler.getInstance().scheduleNext();
        const rt = new RemindersListTool();
        const ui = await rt.execute({ status: 'scheduled' });
...
exports.RemindersUpdateTool = RemindersUpdateTool;
class EmailDraftTool extends types_1.BaseTool {
    name = 'email_draft';
...
exports.EmailDraftTool = EmailDraftTool;
exports.productivityTools = [
    new TasksListTool(),
    new TasksCreateTool(),
