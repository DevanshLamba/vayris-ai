...
export class AgentOrchestrator {
  ...
  async runTask(goal: string | import('../providers/types').MessageContentPart[], context: ToolContext, overrideMode?: 'AUTO' | 'FAST' | 'DEEP'): Promise<TaskState> {
    ...
    if (typeof goal === 'string') {
      ...
      } else if (/^(what are my tasks|show my tasks|list tasks|my tasks|pending tasks|show me my task|show me my tasks)/i.test(lower)) {
        intentAction = { tool: 'tasks_list', args: {} };
      } else if (/^(add|create|new)\s+(?:a\s+)?task\s+(?:of|to|for)?\s*(.+)$/i.test(lower)) {
        const match = lower.match(/^(add|create|new)\s+(?:a\s+)?task\s+(?:of|to|for)?\s*(.+)$/i);
        if (match) intentAction = { tool: 'tasks_create', args: { title: match[2].trim(), dueAt: match[2].trim() } };
      } else if (/^(what are my alarms|show my alarms|list alarms|my alarms|pending alarms)/i.test(lower)) {
        intentAction = { tool: 'alarms_list', args: {} };
      } else if (/^(set|create|add)\s+(?:an\s+)?alarm\s+(?:for|at)?\s*(.+)$/i.test(lower)) {
        const match = lower.match(/^(set|create|add)\s+(?:an\s+)?alarm\s+(?:for|at)?\s*(.+)$/i);
        if (match) intentAction = { tool: 'alarms_create', args: { triggerAt: match[2].trim() } };
      } else if (/^(what are my reminders|show my reminders|list reminders|my reminders|pending reminders)/i.test(lower)) {
        intentAction = { tool: 'reminders_list', args: {} };
      } else if (/^(remind me|create a reminder|add a reminder|set a reminder)\s+(?:to|about)?\s*(.+)$/i.test(lower)) {
        const match = lower.match(/^(remind me|create a reminder|add a reminder|set a reminder)\s+(?:to|about)?\s*(.+)$/i);
        if (match) intentAction = { tool: 'reminders_create', args: { title: match[2].trim(), remindAt: match[2].trim() } };
      } else if (/^mark\s+task\s+(\S+)\s+complete$/i.test(lower)) {
        const match = lower.match(/^mark\s+task\s+(\S+)\s+complete$/i);
        if (match) intentAction = { tool: 'tasks_complete', args: { id: match[1].trim() } };
      } else if (/^(complete|finish|done with)\s+task\s+(\S+)$/i.test(lower)) {
        const match = lower.match(/^(complete|finish|done with)\s+task\s+(\S+)$/i);
        if (match) intentAction = { tool: 'tasks_complete', args: { id: match[2].trim() } };
      } else if (/^(delete|remove)\s+task\s+(\S+)$/i.test(lower)) {
        const match = lower.match(/^(delete|remove)\s+task\s+(\S+)$/i);
        if (match) intentAction = { tool: 'tasks_delete', args: { id: match[2].trim() } };
      }
      ...
      console.log(`[ROUTER] decision=AGENT_TASK (Mode: ${overrideMode || 'AUTO'})`);
    }
    
    const taskMode = overrideMode === 'FAST' ? 'fast' : 'normal';
    const task = this.engine.createTask(goal, taskMode);
    
    // Domain Routing for smaller models (improves tool selection accuracy)
    if (typeof goal === 'string') {
        const lowerGoal = goal.toLowerCase();
        if (/(task|todo|remind|alarm|timer|event|schedule|calendar)/i.test(lowerGoal)) {
            task.metadata.domain = 'productivity';
        }
    }
