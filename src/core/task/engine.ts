...
export class TaskEngine {
  ...
  fastPathResponse(state: TaskState, responseText: string): TaskState {
    ...
    // Emit single delta
    this.emit(TaskEventType.MESSAGE_DELTA, state.taskId, { content: responseText });
  ...
  async fastPathToolExecution(state: TaskState, toolName: string, args: any, context: ToolContext): Promise<TaskState> {
    ...
    if (!permResponse.allowed) {
      ...
      return this.fastPathResponse(state, 'Permission denied.');
    }
  ...
  async runTask(state: TaskState, context: ToolContext): Promise<TaskState> {
    ...
    let memory: Message[] = [
      { 
        role: 'system', 
        content: `You are Vayris, a multi-step execution agent. The current time and date is ${new Date().toLocaleString()}.
...
2. You cannot fabricate or self-attest evidence. Evidence provided to complete_task MUST be based on the actual tool responses you received.` 
    ...
    const allTools = [...this.toolRegistry.getToolDefinitions(), ...controlTools];
    ...
    while (iterations < this.maxIterations) {
      ...
      if (response.tool_calls && response.tool_calls.length > 0) {
        for (const tc of response.tool_calls) {
          ...
          this.emit(TaskEventType.STEP_STARTED, state.taskId, { tool: tc.name });
          this.updateStatus(state, TaskStatus.WAITING_FOR_PERMISSION);
          this.emit(TaskEventType.PERMISSION_REQUESTED, state.taskId, { tool: tc.name });
          ...
          if (!permResponse.allowed) {
            this.emit(TaskEventType.TOOL_RESULT as any, state.taskId, { tool: tc.name, result: { success: false, error: 'Permission denied by user.' } });
            memory.push({ role: 'tool', name: tc.name, tool_call_id: tc.id, content: JSON.stringify({ error: 'Permission denied.' }) });
            continue;
          }
          ...
          this.emit(TaskEventType.PERMISSION_GRANTED, state.taskId, { tool: tc.name });
          this.updateStatus(state, TaskStatus.RUNNING);
          this.emit(TaskEventType.TOOL_CALLED, state.taskId, { tool: tc.name, args: tc.arguments });

          console.log(`[PERF] tool_start (${tc.name})`);
          const result = await this.toolRegistry.executeTool(tc.name, tc.arguments, execContext);
          console.log(`[PERF] tool_end (${tc.name})`);
          
          const emitResult = { ...result };
          if (emitResult.data && emitResult.data._isImage) {
            emitResult.data = { _isImage: true, message: 'Image attached (data hidden for UI performance)' };
          }
          this.emit(TaskEventType.TOOL_RESULT, state.taskId, { tool: tc.name, result: emitResult });
          ...
          if (result.success) {
            ...
            this.emit(TaskEventType.STEP_FAILED, state.taskId, { tool: tc.name, error: result.error });
            repetitiveFails++;
          }
          lastActionStr = actionStr;
        }
